...
import backClass
tpClass = backClass.splitBSback_Class()

#tpClass = splitBSback_Class()
...
class splitBlendshapeUIClass(qw.QMainWindow):
    ...
    def create_ui(self):
        ...
        noLab = qw.QLabel()
        noLab.setText('>> orig << model must have a >> skinCluster <<\nplease paint the weight before click the button~')
        vlayout4.addWidget(noLab)

        spBySkinBtn = qw.QPushButton()
        spBySkinBtn.setText('split by skin weight map')
        vlayout4.addWidget(spBySkinBtn)
        spBySkinBtn.setStyleSheet(Mstyle.QPushButton(kw='b'))
        ...
        spBySkinBtn.clicked.connect(self.spBySkinBtnFn)

    def spBySkinBtnFn(self):
        cmds.undoInfo(openChunk=True)
        sel = self.ogMod

        themap = tpClass.getSkinMapDict(sel, area=self.workDict)

        if themap:
            jntmap = tpClass.convertSkinMapToJointMap(themap)

            lenth = len(jntmap.keys())
            existsArray = []
            for i in range(lenth):
                checkName = self.bsMod + '_splitMesh_' + str(jntmap.keys()[i])
                ...
                dY = - 1.0 * (dTrans[1] + (boundSize[1] * 1.1))
            ...
            for j,ea in zip(jntmap.keys(),existsArray):


                mDict = tpClass.makeExecuteDict(jntmap[j], self.workDict, isInverse=False)

                tpClass.executeVertexMoving(mDict, ea)

            delCheckArray = cmds.ls(self.bsMod + '_splitMesh_*', type = 'transform')
    ...
    def createPlaneBtnFn(self):
        ...
        aaTemp = tpClass.getAreaDict(self.bsMod, self.spPlanes, self.workDict)
        self.areaArray = aaTemp['area']
